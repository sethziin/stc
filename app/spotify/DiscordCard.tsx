...
export default function DiscordCard() {
  ...
  useEffect(() => {
    ...
    ws.onopen = () => ws.send(JSON.stringify({ op: 2, d: { subscribe_to_id: profile.discord_id } }));
    ws.onmessage = (e) => {
    ...
    >
      <div className="flex items-center gap-4">
        <div className="relative">
          ...
          />
          <span className={`absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-[#0b0b0b] ${statusColor}`} />
        </div>
        ...
        <div className="leading-tight">
          <div className="flex items-center gap-1">
            ...
            {profile.badges.map((b) =>
              badgeIcons[b.key] ? (
                <img key={b.key} src={badgeIcons[b.key]} title={b.label} className="w-4 h-4 opacity-80 hover:opacity-100 transition" />
              ) : null
            )}
          </div>
      ...
      {song && albumArt && (
        <img src={albumArt} alt="album" className="w-14 h-14 rounded-xl object-cover shadow-md animate-fade-in" />
      )}
